...
def saveData():
    ...
    np.savez(f'{save_path}/%d.npz' % matchid
    ...
    with open(f'{data_path}/{file}', encoding='UTF-8') as f:
        ...
        while line:
            ...
            if t[0] == 'Match':
                ...
                matchid += 1
                if matchid % 128 == 0:
                    print(f'[{file}]:Processing match %d %s...' % (matchid, t[1]))
            elif t[0] == 'Wind':
                for agent in agents:
...
for file in os.listdir(data_path):
    print(file)
    if file not in ['data-BTW.txt', 'data-WTB.txt']:
        continue
    ids.append((file, matchid))
    process(file)
print (ids)
# process('sample-BTW.txt')
# print (l)
# print (matchid, 'matches')
